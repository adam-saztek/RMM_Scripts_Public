...
#Description: Installs network printers and deletes old ones.  This script gets the print drivers from a web server.  Print drivers can be bigger than the size allowed by Syncro, and this 
#script only downloads the drivers needed.  You will have to specify the .inf file for the print driver (usually only one, easy to find) and the name of the printer driver in the .inf, usually
#listed at the top of the .inf.  The driver files should be saved on the web server as the same name as the printName variable + .zip, so "Some Printer Name.zip".  If the printer has more complex
...
foreach($printer in $activePrinters)
{
    ...
    if ($validWorkstation)
    {
        ...
        if (!(get-printer "$printerName*"))
        {
            ...
            $printerXmlURL = -join("https://your.fileserver.com/printers/",$printerXmlFileName)
